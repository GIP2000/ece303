...
def add_hash(frame,i,logger=False):
    """
    takes a frame of size 1016 and adds a hase of size 16
    """
    # m.update(frame)
    # hsh = m.digest()
    hsh = fletcher32(frame,len(frame))
...
def get_frame_size(data_bytes):
    l = len(data_bytes) / 1016
    return l+1 if len(data_bytes) % 1016 else l
...
def slice_frames(data_bytes):
    ...
    extra = 1 if num_bytes % 1016 else 0

    for i in xrange(num_bytes / 1016 + extra):
...
def check_checksum(data,logger=False):
    ...
    # m.update(real_data)
    # my_hsh = m.digest()
    my_hsh = fletcher32(real_data,len(real_data))
    return real_data,hsh != my_hsh,hsh,index


def fletcher32(data, length=False):
    if not length:
        length = len(data)
    c0 = 0
    c1 = 0
